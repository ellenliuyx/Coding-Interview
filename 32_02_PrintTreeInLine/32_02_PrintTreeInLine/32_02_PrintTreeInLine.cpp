...
void PrintTree(BinaryTreeNode* p_Root)
{
	...
	while (!myQueue.empty())			//直到队列为空是结束循环
	{
		...
		if (!toBePrinted)								//若本层已经没有需要打印的节点了
		{
			cout << endl;								//输出换行符
			toBePrinted = nextLevel;					//将nextLevel赋给toBePrinted
			nextLevel = 0;								//nextLevel置0重新计数
		}
	}
}
